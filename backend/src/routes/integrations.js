...
import { readUploadBuffer, safeUploadFilename, saveUploadBuffer } from "../services/storageService.js";

const router = express.Router();
const PORT = Number(process.env.PORT || 4000);

router.post("/upload-file", requireAuth, async (req, res) => {
  const filename = safeUploadFilename(req.body.name);
...
router.get("/uploads/:filename", async (req, res) => {
  const file = await readUploadBuffer(req.params.filename);
  if (!file) return res.redirect(`/uploads/${encodeURIComponent(req.params.filename)}`);
